...
# The data can then be loaded with the rake db:seed:field_control_files (or created alongside the db with db:setup).
#
# Examples:
#
...
AppEnv.create(:id => 28, :key => 'FILE_DIR', :value => '/var/deco/files', :category => 0)
AppEnv.create(:id => 29, :key => 'PASSWORD_AUTOMATION', :value => '0', :category => 0)
AppEnv.create(:id => 30, :key => 'ENABLE_SSL', :value => '0', :category => 0)
AppEnv.create(:id => 31, :key => 'VIRUS_CHECK', :value => '1', :category => 0)
AppEnv.create(:id => 32, :key => 'VIRUS_CHECK_NOTICE', :value => '0', :category => 0)
